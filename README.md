...
Download the newest Kernel Version ([6.12.*](https://github.com/Rappelkiste98/raspberry-linux-pve/tree/6.12.y))
```bash
...
wget -q --show-progress https://github.com/Rappelkiste98/raspberry-linux-pve/releases/download/6.12.34/linux-headers-6.12.34-v8-pve+_arm64.deb
```
Install new Kernel Image
```bash
sudo apt install $PWD/linux-image-6.12.34-v8-pve+_arm64.deb
```

Install new Kernel Headers
```bash
sudo apt install $PWD/linux-headers-6.12.34-v8-pve+_arm64.deb
```
...
sudo zpool --version => 'zfs-2.2.6-pve1 zfs-kmod-2.3.4-1'
```
